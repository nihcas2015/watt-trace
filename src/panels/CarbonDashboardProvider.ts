...
import { AnalysisResult, FunctionAnalysis, CarbonBreakdown } from '../../carbonFootprint/models';
import { OllamaService, GreenSuggestion } from '../services/ollamaService';
import { formatCo2 } from '../extension';

export class CarbonDashboardProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'watttrace.carbonDashboard';

  private _view?: vscode.WebviewView;
  private _result: AnalysisResult | null = null;
  private _isAnalyzing = false;
  private _pendingSuggestion: string | null = null; // fn name being processed
  ...
  private async handleSuggestFix(
    functionName: string,
    line: number,
  ): Promise<void> {
    ...
    const fnData = this._result.functions.find(
      (f) => f.name === functionName && f.lineNumber === line,
    );
    if (!fnData) {
    ...
    if (!alive) {
      vscode.window.showErrorMessage(
        'WattTrace: Cannot reach Ollama. Make sure it is running (ollama serve).',
      );
      return;
    }
    ...
    try {
      const suggestion = await vscode.window.withProgress(
        ...
        () =>
          this.ollamaService.suggestGreenerCode(
            functionCode,
            functionName,
            this._result!.language,
            fnData.carbonGrams,
            fnData.weightedOps,
          ),
      );
  ...
  private extractFunctionCode(
    ...
    if (startLine < 0 || startLine >= doc.lineCount) { return null; }
  ...
  private extractPythonFunction(
    ...
    for (let i = startLine + 1; i < endLine; i++) {
      ...
      if (lt.trim() === '') { lines.push(lt); continue; }
...
`,
  ...
  private revealFunction(line: number): void {
    const editor = vscode.window.activeTextEditor;
...
<body>
  ...
  private renderOllamaLoading(): string {
    return /* html */ `
  ...
  private renderOverview(r: AnalysisResult): string {
    const bd = r.carbonBreakdown;
    const carbonDisplay = formatCo2(bd.total.carbonGrams);
  ...
  private renderBreakdown(r: AnalysisResult): string {
    const bd = r.carbonBreakdown;
    ...
    const rows = tiers.map(({ tier, icon, color, delay }) => {
      ...
      const co2 = formatCo2(tier.carbonGrams);
      return /* html */ `
  ...
  private renderFunctionList(r: AnalysisResult): string {
    ...
    const rows = sorted.map((fn, i) => {
      ...
      const carbonStr = formatCo2(fn.carbonGrams);
  ...
  private getGrade(carbonGrams: number): string {
    // Thresholds based on daily total across all three tiers
    if (carbonGrams <= 1e-4) { return 'A'; }
    if (carbonGrams <= 1e-2) { return 'B'; }
    if (carbonGrams <= 1)    { return 'C'; }
    if (carbonGrams <= 100)  { return 'D'; }
    return 'F';
  }

  private getFunctionRisk(fn: FunctionAnalysis, totalWeighted: number): string {
    const pct = (fn.weightedOps / totalWeighted) * 100;
    if (pct > 40 || fn.maxNesting > 3) { return 'high'; }
    if (pct > 15 || fn.maxNesting > 2) { return 'medium'; }
    return 'low';
  }
}

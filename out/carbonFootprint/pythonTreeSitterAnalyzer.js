...
exports.PythonTreeSitterAnalyzer = void 0;
...
class PythonTreeSitterAnalyzer {
    ...
    constructor() {
        this.ioFunctions = constants_1.IO_FUNCTIONS['python'] || new Set();
        this.networkFunctions = constants_1.NETWORK_FUNCTIONS['python'] || new Set();
        this.allocFunctions = constants_1.ALLOC_FUNCTIONS['python'] || new Set();
        this.ioCallSubstrings = constants_1.IO_CALL_SUBSTRINGS['python'] || [];
        this.networkCallSubstrings = constants_1.NETWORK_CALL_SUBSTRINGS['python'] || [];
    }
    /**
    ...
    analyze(rootNode, filePath) {
        this.result = new models_1.AnalysisResult('python', filePath || null);
        ...
        for (const child of rootNode.namedChildren) {
            ...
            if (nodeType === 'function_definition') {
                ...
                const innerDef = child.namedChildren.find((c) => c.type === 'function_definition' || c.type === 'class_definition');
                if (innerDef) {
    ...
    resolveConstantExpr(node) {
        ...
        if (node.type === 'unary_operator') {
            const operand = node.namedChildren.find((c) => c.type === 'integer' || c.type === 'float' || c.type === 'identifier');
    ...
    walkForCalls(node, func, funcName) {
        if (node.type === 'call') {
            const callName = this.getCallName(node);
            if (callName) {
    ...
    analyzeNode(node, loopMultiplier) {
        ...
        switch (node.type) {
            ...
            case 'augmented_assignment': {
                ...
                switch (augOp) {
                    case '+=':
                        ops.add(constants_1.OpType.ADDITION, loopMultiplier);
                        break;
                    case '-=':
                        ops.add(constants_1.OpType.SUBTRACTION, loopMultiplier);
                        break;
                    case '*=':
                    case '@=':
                        ops.add(constants_1.OpType.MULTIPLICATION, loopMultiplier);
                        break;
                    case '/=':
                    case '//=':
                    case '%=':
                        ops.add(constants_1.OpType.DIVISION, loopMultiplier);
                        break;
                }
                break;
            }
            ...
            case 'for_statement': {
                ...
                const iterations = iterable
                    ...
                    : constants_1.DEFAULT_LOOP_ITERATIONS;
                const innerMultiplier = loopMultiplier * iterations;
                if (iterations !== constants_1.DEFAULT_LOOP_ITERATIONS) {
                    this.result.assumptions.push(`Line ${node.startPosition.row + 1}: for-loop resolved to ${iterations} iterations`);
                }
                else {
                    this.result.assumptions.push(`Line ${node.startPosition.row + 1}: for-loop iterations unknown, assumed ${constants_1.DEFAULT_LOOP_ITERATIONS}`);
                }
            ...
            case 'while_statement': {
                ...
                const iterations = condition
                    ...
                    : constants_1.DEFAULT_LOOP_ITERATIONS;
                const innerMultiplier = loopMultiplier * iterations;
                this.result.assumptions.push(`Line ${node.startPosition.row + 1}: while-loop estimated ${iterations} iterations`);
                ops.add(constants_1.OpType.COMPARISON, loopMultiplier * iterations);
                if (condition) {
            ...
            case 'delete_statement': {
                ops.add(constants_1.OpType.MEMORY_ALLOC, loopMultiplier); // deallocation cost
                break;
            }
            ...
            case 'raise_statement': {
                ops.add(constants_1.OpType.FUNCTION_CALL, loopMultiplier); // exception overhead
                break;
            }
            // --- Assert ---
            case 'assert_statement': {
                ops.add(constants_1.OpType.COMPARISON, loopMultiplier);
                for (const child of node.namedChildren) {
                    ops.merge(this.analyzeExpression(child, loopMultiplier));
                }
                break;
    ...
    analyzeExpression(node, multiplier = 1) {
        ...
        switch (node.type) {
            ...
            case 'binary_operator': {
                ...
                switch (operator) {
                    ...
                    case '**':
                        ...
                        ops.add(constants_1.OpType.MULTIPLICATION, multiplier * 10);
                        break;
                    default:
            ...
            case 'comparison_operator': {
                ...
                ops.add(constants_1.OpType.COMPARISON, multiplier * Math.max(1, compCount));
            ...
            case 'boolean_operator': {
                ...
                if (boolLeft) {
                    ops.merge(this.analyzeExpression(boolLeft, multiplier));
                }
                if (boolRight) {
                    ops.merge(this.analyzeExpression(boolRight, multiplier));
                }
                break;
            }
            // --- Not operator ---
            case 'not_operator': {
                ...
                if (notArg) {
                    ops.merge(this.analyzeExpression(notArg, multiplier));
                }
                break;
            }
            // --- Function calls ---
            case 'call': {
                ...
                if (callName) {
                    if (this.ioFunctions.has(callName) ||
                        ...
                        ops.add(constants_1.OpType.COMPARISON, multiplier * constants_1.DEFAULT_LOOP_ITERATIONS * 7);
                        ops.add(constants_1.OpType.ASSIGNMENT, multiplier * constants_1.DEFAULT_LOOP_ITERATIONS * 7);
                    }
                    else if (['sum', 'min', 'max', 'any', 'all'].includes(callName)) {
                        // These iterate over their argument — O(n)
                        ops.add(constants_1.OpType.ADDITION, multiplier * constants_1.DEFAULT_LOOP_ITERATIONS);
                        ops.add(constants_1.OpType.COMPARISON, multiplier * constants_1.DEFAULT_LOOP_ITERATIONS);
                    }
                ...
                if (argsNode) {
                    for (const arg of argsNode.namedChildren) {
                        if (arg.type === 'keyword_argument') {
                            ...
                            if (valNode) {
                                ops.merge(this.analyzeExpression(valNode, multiplier));
                            }
                        }
                        else {
            ...
            case 'subscript': {
                ...
                if (subValue) {
                    ops.merge(this.analyzeExpression(subValue, multiplier));
                }
                if (subSlice) {
                    ops.merge(this.analyzeExpression(subSlice, multiplier));
                }
                break;
            }
            ...
            case 'generator_expression': {
                ...
                ops.add(constants_1.OpType.MEMORY_ALLOC, multiplier); // creating the collection
                ...
                for (const child of node.namedChildren) {
                    if (child.type === 'for_in_clause') {
                        ops.add(constants_1.OpType.COMPARISON, innerMult);
                        const iterExpr = child.childForFieldName('value') || child.namedChildren.find((c) => c.type !== 'identifier' && c.type !== 'pattern_list');
                        if (iterExpr) {
                            ops.merge(this.analyzeExpression(iterExpr, multiplier));
                        }
                    }
                    else if (child.type === 'if_clause') {
                        ops.add(constants_1.OpType.CONDITIONAL, innerMult);
            ...
            case 'dictionary_comprehension': {
                ...
                if (pair) {
                    ...
                    if (key) {
                        ops.merge(this.analyzeExpression(key, dictInnerMult));
                    }
                    if (value) {
                        ops.merge(this.analyzeExpression(value, dictInnerMult));
                    }
                }
                for (const child of node.namedChildren) {
                    if (child.type === 'for_in_clause') {
            ...
            case 'unary_operator': {
                ...
                if (operand) {
                    ops.merge(this.analyzeExpression(operand, multiplier));
                }
                break;
            }
            // --- Attribute access ---
            case 'attribute': {
                ...
                if (attrObj) {
                    ops.merge(this.analyzeExpression(attrObj, multiplier));
                }
                break;
            }
            ...
            case 'dictionary': {
                ...
                if (pairs.length > 0) {
                    ops.add(constants_1.OpType.MEMORY_ALLOC, multiplier);
                    ops.add(constants_1.OpType.ASSIGNMENT, multiplier * pairs.length);
                }
                for (const p of pairs) {
                    const key = p.childForFieldName('key') || p.namedChildren[0];
                    const value = p.childForFieldName('value') || p.namedChildren[1];
                    if (key) {
                        ops.merge(this.analyzeExpression(key, multiplier));
                    }
                    if (value) {
                        ops.merge(this.analyzeExpression(value, multiplier));
                    }
                }
            ...
            case 'dictionary_splat': {
                ...
                if (splatExpr) {
                    ops.merge(this.analyzeExpression(splatExpr, multiplier));
                }
                break;
            }
            // --- Parenthesized expression ---
            ...
            case 'await': {
                ...
                if (awaitExpr) {
                    ops.merge(this.analyzeExpression(awaitExpr, multiplier));
                }
                break;
            }
            // --- Yield expression ---
            case 'yield': {
                ...
                if (yieldExpr) {
                    ops.merge(this.analyzeExpression(yieldExpr, multiplier));
                }
                break;
            }
            // --- Lambda ---
            case 'lambda': {
                ...
                if (lambdaBody) {
                    ops.merge(this.analyzeExpression(lambdaBody, multiplier));
                }
                break;
            }
            // --- Walrus operator (:=) ---
            case 'named_expression': {
                ...
                if (namedValue) {
                    ops.merge(this.analyzeExpression(namedValue, multiplier));
                }
                break;
            }
     ...
     */
    estimateForIterations(iterable, _forNode) {
        // range() call
        if (iterable.type === 'call') {
            ...
            if (callName === 'range') {
                ...
                if (args.length === 3 && resolvedArgs.every((r) => r !== undefined)) {
                    const [start, stop, step] = resolvedArgs;
                    if (step !== 0) {
        ...
        if (iterable.type === 'string') {
            ...
            return content.length || constants_1.DEFAULT_LOOP_ITERATIONS;
        }
     ...
     * - Fallback to DEFAULT_LOOP_ITERATIONS
     */
    estimateWhileIterations(condition, whileNode) {
     ...
     */
    estimateComprehensionIterations(node) {
    ...
    getCallArguments(callNode) {
        const argsNode = callNode.childForFieldName('arguments');
        if (!argsNode) {
            return [];
        }
        return argsNode.namedChildren.filter((c) => c.type !== 'keyword_argument' && c.type !== 'dictionary_splat' && c.type !== 'list_splat');
    }
    ...
    parseIntLiteral(text) {
        const cleaned = text.replace(/_/g, '');
        const val = Number(cleaned);
        return isNaN(val) ? undefined : Math.floor(val);
    }
}
exports.PythonTreeSitterAnalyzer = PythonTreeSitterAnalyzer;
//# sourceMappingURL=pythonTreeSitterAnalyzer.js.map
...
const IO_PATTERNS: Record<string, RegExp> = {
...
export class RegexAnalyzer {
  ...
  private readonly ioPattern: RegExp | null;
  private readonly networkPattern: RegExp | null;
  private readonly allocPattern: RegExp | null;
  private readonly funcPattern: RegExp | null;
  private readonly isBraceBased: boolean;
  ...
  analyze(code: string, filePath?: string): AnalysisResult {
    ...
    this.result.assumptions.push(
      `Regex-based analysis (tree-sitter unavailable) for ${this.language}`,
    );
    this.result.assumptions.push(
  ...
  private extractConstants(code: string): void {
    if (this.language === 'python') {
      // Python: NAME = NUMBER
  ...
  private extractPythonFunctions(code: string): void {
    const lines = code.split('\n');
    let i = 0;
    ...
    while (i < lines.length) {
      ...
      if (defMatch) {
        ...
        const func = new FunctionAnalysis(funcName, startLine);
        ...
        if (func.isRecursive) {
          ...
          this.result.assumptions.push(
            `Function '${funcName}' is recursive — assumed ${DEFAULT_RECURSION_DEPTH} recursive calls`,
          );
        }
  ...
  private analyzePythonBlock(code: string, ops: OperationCount, multiplier: number): void {
    const lines = code.split('\n');
    let i = 0;
    ...
    while (i < lines.length) {
      ...
      if (forMatch) {
        ...
        this.result.assumptions.push(
          `Line ${i + 1}: for-loop estimated ${iterations} iterations`,
        );

        ops.add(OpType.COMPARISON, multiplier * iterations);
      ...
      if (whileMatch) {
        ...
        this.result.assumptions.push(
          `Line ${i + 1}: while-loop estimated ${iterations} iterations`,
        );
  ...
  private extractBraceFunctions(code: string): void {
    if (!this.funcPattern) {
    ...
    for (const fm of allMatches) {
      ...
      const func = new FunctionAnalysis(fm.name, lineNumber);
      ...
      if (func.isRecursive) {
        func.operations = func.operations.scale(DEFAULT_RECURSION_DEPTH);
        this.result.assumptions.push(
          `Function '${fm.name}' is recursive — assumed ${DEFAULT_RECURSION_DEPTH} recursive calls`,
        );
      }
   ...
   */
  private extractBraceBlock(code: string, startIdx: number): string {
    let depth = 0;
    let i = startIdx;
   ...
   */
  private analyzeCodeByDepth(code: string, ops: OperationCount, baseMultiplier: number): void {
    const lines = code.split('\n');
    const loopStack: Array<{ iterations: number; braceDepth: number }> = [];
    let braceDepth = 0;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (forMatch && !foreachMatch) {
        ...
        ops.add(OpType.COMPARISON, currentMultiplier * iterations);
        this.result.assumptions.push(
          `Line ${i + 1}: for-loop estimated ${iterations} iterations`,
        );
      } else if (foreachMatch) {
        loopStack.push({ iterations: DEFAULT_LOOP_ITERATIONS, braceDepth });
        ops.add(OpType.COMPARISON, currentMultiplier * DEFAULT_LOOP_ITERATIONS);
        this.result.assumptions.push(
          `Line ${i + 1}: for-each loop assumed ${DEFAULT_LOOP_ITERATIONS} iterations`,
        );
      } else if (whileMatch) {
        ...
        ops.add(OpType.COMPARISON, currentMultiplier * DEFAULT_LOOP_ITERATIONS);
        this.result.assumptions.push(
          `Line ${i + 1}: do-while loop assumed ${DEFAULT_LOOP_ITERATIONS} iterations`,
        );
      } else {
        // Regular line — count operations
   ...
   */
  private countLineOperations(line: string, ops: OperationCount, multiplier: number): void {
    ...
    if (assignments) {
      ops.add(OpType.ASSIGNMENT, multiplier * assignments.length);
    }

    // Comparisons (==, !=, <, >, <=, >=)
    const comparisons = line.match(/(==|!=|<=|>=|<|>)/g);
    if (comparisons) {
      ops.add(OpType.COMPARISON, multiplier * comparisons.length);
    }
    ...
    if (arrayAccess) {
      ops.add(OpType.ARRAY_ACCESS, multiplier * arrayAccess.length);
    }
   ...
   */
  private estimateForIterationsFromHeader(header: string): number {
    ...
    if (standardMatch) {
      ...
      switch (op) {
        ...
        default: return DEFAULT_LOOP_ITERATIONS;
      }
    }
   ...
   */
  private estimatePythonForIterations(iterable: string): number {
    // range(N)
    ...
    if (range3Match) {
      ...
      return step > 0 ? Math.max(0, Math.ceil((stop - start) / step)) : DEFAULT_LOOP_ITERATIONS;
    }
   ...
   */
  private estimateWhileIterationsFromCondition(header: string): number {
   ...
   */
  private estimatePythonWhileIterations(condition: string): number {
    ...
    if (match) {
      ...
      return endVal > 0 ? endVal : DEFAULT_LOOP_ITERATIONS;
    }
  ...
  private countPythonNesting(code: string): number {
    let maxDepth = 0;
    let currentDepth = 0;
    const lines = code.split('\n');
  ...
  private countBraceNesting(code: string): number {
    let maxDepth = 0;
    let currentDepth = 0;
